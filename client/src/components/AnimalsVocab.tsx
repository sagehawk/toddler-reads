...
const findPhonicsData = (word: string) => {
  ...
  for (const module of learningModules) {
    if (module.type === 'cvc' && module.words) {
      const found = module.words.find(w => w.word.toLowerCase() === lowerWord);
      if (found) {
        return {
          consonantSound: module.letters?.find(l => l.letter === found.consonant)?.sound 
             || learningModules[0].letters?.find(l => l.letter === found.consonant)?.sound,
  ...
  return {
    consonantSound: learningModules[0].letters?.find(l => l.letter === firstLetter)?.sound,
...
export const AnimalsVocab = ({ items, onExit }: AnimalsVocabProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#FFFAF0] flex flex-col font-nunito">
      {/* Top Bar */}
      <div className="p-4">
        <button onClick={onExit} className="p-2 rounded-full bg-white shadow-sm hover:shadow-md transition-all">
